...
func TestRedisMutations(t *testing.T) {
	...
	t.Run("hash list set zset and stream operations use argument arrays", func(t *testing.T) {
		...
		assert.Equal(t, []any{"SADD", "s", "member with spaces"}, exec.calls[4])
		assert.Equal(t, []any{"SREM", "s", "member with spaces"}, exec.calls[5])
		assert.Equal(t, []any{"ZADD", "z", 1.5, "member with spaces"}, exec.calls[6])
		assert.Equal(t, []any{"ZREM", "z", "member with spaces"}, exec.calls[7])
		assert.Equal(t, []any{"XADD", "x", "*", "name", "alice bob"}, exec.calls[8])
		assert.Equal(t, []any{"XDEL", "x", "1-0"}, exec.calls[9])
	})
}
